document.addEventListener('DOMContentLoaded', function() {
  ...
  async function loadDynamicNavMenu() {
    ...
    try {
      ...
      // Helper to convert backend clean URLs to static html params
      function convertLink(url) {
        ...
        if (url.startsWith('/category/')) {
          var parts = url.split('/');
          if (parts.length === 3) {
            return 'category.html?cat=' + parts[2];
          } else if (parts.length >= 5 && parts[3] === 'sub') {
            return 'category.html?cat=' + parts[2] + '&sub=' + parts[4];
          }
        } else if (url.startsWith('/')) {
          return url.substring(1) + '.html'; // e.g. /jobs -> jobs.html
        }
        return url;
      }
      ...
      menus.forEach(function(menu) {
        ...
        if (hasDropdown) {
          html += '<li class="nav-item has-dropdown ' + isActive + '">' +
            '<a href="' + convertLink(menu.linkUrl) + '" class="nav-link">' + sanitize(displayName) + ' <i class="fas fa-chevron-down" style="font-size:0.7em"></i></a>' +
            '<ul class="mega-menu">';
          menu.subcategories.forEach(function(sub) {
            ...
            html += '<li><a href="' + convertLink(sub.linkUrl) + '" class="nav-link">' + sanitize(subName) + '</a></li>';
          });
          html += '</ul></li>';
        } else {
          html += '<li class="nav-item ' + isActive + '">' +
            '<a href="' + convertLink(menu.linkUrl) + '" class="nav-link">' + sanitize(displayName) + '</a></li>';
        }
      });
  ...
  async function loadBreakingNews() {
    ...
    // Fallback 3+ items
    const dummyBreaking = [
      '🌾 நெல் கொள்முதல் விலை குவிண்டாலுக்கு ₹100 உயர்வு - விவசாயிகள் மகிழ்ச்சி',
      '⚡ தமிழகத்தில் மின்சார உள்கட்டமைப்பு மேம்பாட்டிற்கு ₹5,000 கோடி நிதி ஒதுக்கீடு',
      '🚆 சென்னையில் 2-வது மெட்ரோ வழித்தடப் பணிகள் தீவிரம் - தெற்கு ரயில்வே செய்தி'
    ];
    track.innerHTML = dummyBreaking.map(t => `<a href="#">${sanitize(t)}</a>`).join('');
  }
  ...
  async function loadHeroSection() {
    ...
    if (!articles || articles.length === 0) {
      articles = [
        { id: 1, titleTa: 'தமிழகத்தில் விரைவில் சட்டமன்ற கூட்டத்தொடர் – முதலமைச்சர் அறிவிப்பு', shortDescTa: 'தமிழக சட்டமன்றக் கூட்டத்தொடர் விரைவில் தொடங்க உள்ளது. இக்கூட்டத்தொடரில் பல்வேறு முக்கிய மசோதாக்கள் விவாதத்திற்கு வர உள்ளன.', publishedAt: new Date().toISOString(), viewsCount: 5420, imageUrl: 'https://images.unsplash.com/photo-1541872703-74c5e44368f9?w=800' },
        { id: 2, titleTa: 'பங்குச்சந்தை புதிய உச்சம் – முதலீட்டாளர்கள் மகிழ்ச்சி', shortDescTa: 'வர்த்தக இறுதியில் சென்செக்ஸ் 600 புள்ளிகள் உயர்ந்தது.', publishedAt: new Date().toISOString(), viewsCount: 3120 },
        { id: 3, titleTa: 'IPL 2026: சென்னை அணி அபார வெற்றி', shortDescTa: 'இறுதி ஓவரில் சென்னை அணி வெற்றி இலக்கை எட்டியது.', publishedAt: new Date().toISOString(), viewsCount: 8900 },
        { id: 4, titleTa: 'இந்தியா விண்வெளியில் புதிய சாதனை – இஸ்ரோ திட்டம் வெற்றி', shortDescTa: 'புதிய செயற்கைக்கோள் வெற்றிகரமாக சுற்றுப்பாதையில் நிலைநிறுத்தம்.', publishedAt: new Date().toISOString(), viewsCount: 4200 }
      ];
    }
  ...
  // ===== SHARED HELPER: Render news cards into a grid =====
  function renderNewsCards(articles, gridId, catBadgeColor, catNameFallback) {
    const grid = document.getElementById(gridId);
    if (!grid) return;
    if (!articles || articles.length === 0) {
      grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:30px;color:var(--text-muted);font-size:14px;">${catNameFallback ? catNameFallback + ' ' : ''}\u0b9a\u0bc6\u0baf\u0bcd\u0ba4\u0bbf\u0b95\u0bb3\u0bcd \u0b8f\u0ba4\u0bc1\u0bae\u0bcd \u0b87\u0bb2\u0bcd\u0bb2\u0bc8.</div>`;
      return;
    }
    const lang = Storage.get('lang', 'ta');
    grid.innerHTML = articles.map(art => {
      ...
      const catName = lang === 'ta' ? (art.categoryNameTa || art.categoryName || catNameFallback || '') : (art.categoryName || art.categoryNameTa || catNameFallback || '');
      const imgUrl = getMediaUrl(art.thumbnailUrl || art.featuredImage || art.imageUrl);
      const bgStyle = imgUrl ? `background-image:url('${imgUrl}');background-size:cover;background-position:center;` : `background:linear-gradient(135deg,${catBadgeColor}99,${catBadgeColor});`;
      return `
        <div class="news-card" onclick="window.location.href='article.html?id=${art.id}'" style="cursor:pointer;">
          <div class="card-img" style="${bgStyle}">
            <span class="cat-badge" style="background:${catBadgeColor};">${sanitize(catName)}</span>
          </div>
          <div class="card-body">
            <h3>${sanitize(title || '')}</h3>
            <p>${sanitize(desc ? desc.substring(0, 100) + '...' : '')}</p>
            <div class="card-meta">
              <span><i class="far fa-clock"></i> ${formatDate(art.publishedAt || art.createdAt)}</span>
  ...
  let _navCategories = null;
  async function getNavCategories() {
    if (_navCategories) return _navCategories;
    try {
      const res = await fetch(`${API_BASE}/api/v1/categories/nav`);
      if (!res.ok) return [];
      _navCategories = await res.json();
      return _navCategories;
    } catch { return []; }
  }
  ...
  async function getCategoryIdBySlug(slug) {
    ...
    for (const cat of cats) {
      ...
      const catNameTa = (cat.nameTa || '').toLowerCase();
      if (catSlug === slugLower || catName === slugLower || catName.includes(slugLower) || catNameTa.includes(slugLower)) {
        return cat.id;
      }
    }
    return null;
  ...
  async function loadLatestNews() {
    ...
    if (!articles || articles.length === 0) {
      articles = [
        { id: 10, titleTa: 'தமிழக சட்டப்பேரவையில் புதிய மசோதா தாக்கல் – முக்கிய திருத்தங்கள்', shortDescTa: 'சட்டப்பேரவையில் இன்று தாக்கல் செய்யப்பட்ட புதிய மசோதாவுக்கு பல தரப்பில் வரவேற்பு கிடைத்துள்ளது.', publishedAt: new Date().toISOString(), viewsCount: 4500 },
        { id: 11, titleTa: 'இந்திய கிரிக்கெட் அணி ஆஸ்திரேலியாவை வீழ்த்தியது – 3-0 அபார தொடர் வெற்றி', shortDescTa: 'ஆஸ்திரேலியாவுக்கு எதிரான டி20 தொடரை 3-0 என்ற கணக்கில் இந்திய அணி முழுமையாக வென்றது.', publishedAt: new Date().toISOString(), viewsCount: 18200 },
        { id: 12, titleTa: 'பங்குச் சந்தை புதிய உச்சம் – முதலீட்டாளர்களுக்கு மகிழ்ச்சியான செய்தி', shortDescTa: 'வார இறுதி வர்த்தகத்தில் அனைத்து துறை பங்குகளும் ஏற்றத்தில் நிறைவடைந்தன.', publishedAt: new Date().toISOString(), viewsCount: 6100 },
        { id: 13, titleTa: 'சென்னையில் புதிய உயர்மட்ட மேம்பாலம் திறப்பு – போக்குவரத்து நெரிசல் குறையும்', shortDescTa: 'அண்ணாசாலையில் அமைக்கப்பட்ட புதிய மேம்பாலத்தை முதலமைச்சர் திறந்து வைத்தார்.', publishedAt: new Date().toISOString(), viewsCount: 3800 },
        { id: 14, titleTa: '+2 பொதுத்தேர்வு முடிவுகள் ஜூன் 5-இல் வெளியீடு – கல்வித்துறை தகவல்', shortDescTa: 'தேர்வு தாள்கள் திருத்தும் பணி நிறைவடைந்த நிலையில் அதிகாரப்பூர்வ அறிவிப்பு.', publishedAt: new Date().toISOString(), viewsCount: 9200 },
        { id: 15, titleTa: 'தமிழகத்தில் 5 மாவட்டங்களுக்கு கனமழை எச்சரிக்கை – வானிலை மையம்', shortDescTa: 'தென்மேற்கு பருவமழை காரணமாக அடுத்த 48 மணி நேரத்திற்கு கனமழை பெய்ய வாய்ப்பு.', publishedAt: new Date().toISOString(), viewsCount: 12400 }
      ];
    }
  ...
  async function loadVideoNews() {
    ...
    if (!videos || videos.length === 0) {
      videos = [
        { id: 1, title: 'KINGS 24x7 நேரலை செய்தி அறிக்கை | Tamil News Broadcast', youtubeUrl: 'https://www.youtube.com/watch?v=2g811Eo7K8U', duration: '12:45', viewsCount: 15400 },
        { id: 2, title: 'தமிழக பட்ஜெட் 2026 முக்கிய அறிவிப்புகள் | Special Report', youtubeUrl: 'https://www.youtube.com/watch?v=2g811Eo7K8U', duration: '08:30', viewsCount: 9200 },
        { id: 3, title: 'விவசாயிகள் பிரத்யேக நேர்காணல் | Agro Special Story', youtubeUrl: 'https://www.youtube.com/watch?v=2g811Eo7K8U', duration: '15:10', viewsCount: 6800 },
        { id: 4, title: 'சென்னை வானிலை நிலவரம் நேரடி காட்சி | Weather Bulletin', youtubeUrl: 'https://www.youtube.com/watch?v=2g811Eo7K8U', duration: '05:20', viewsCount: 11300 }
      ];
    }
  ...
  async function loadWebStories() {
    ...
    if (!stories || stories.length === 0) {
      stories = [
        { id: 1, titleTa: 'உலக கோப்பை கிரிக்கெட் அட்டவணை 2027', badge: 'NEW', cat: 'விளையாட்டு', backgroundGradient: 'linear-gradient(135deg, #667eea, #764ba2)', viewsCount: 12400 },
        { id: 2, titleTa: 'ரஜினி அடுத்த படம் - பிரம்மாண்ட பார்வை', badge: 'HOT', cat: 'சினிமா', backgroundGradient: 'linear-gradient(135deg, #D946EF, #EC4899)', viewsCount: 18200 },
        { id: 3, titleTa: 'சட்டமன்ற தேர்தல் 2026 - முன்னோட்டம்', badge: 'TREND', cat: 'அரசியல்', backgroundGradient: 'linear-gradient(135deg, #1E40AF, #3B82F6)', viewsCount: 9500 },
        { id: 4, titleTa: 'AI மருத்துவ புரட்சி - புதிய கண்டறிதல்', badge: 'NEW', cat: 'தொழில்நுட்பம்', backgroundGradient: 'linear-gradient(135deg, #7C3AED, #A855F7)', viewsCount: 15000 },
        { id: 5, titleTa: 'கரிம வேளாண்மை - விவசாயி வெற்றி கதை', badge: 'NEW', cat: 'விவசாயம்', backgroundGradient: 'linear-gradient(135deg, #16A34A, #4ADE80)', viewsCount: 8400 },
        { id: 6, titleTa: 'புதிய முதலீட்டு வாய்ப்புகள் 2026', badge: 'TREND', cat: 'வணிகம்', backgroundGradient: 'linear-gradient(135deg, #059669, #22C55E)', viewsCount: 11200 }
      ];
    }
  ...
  async function loadTrendingNews() {
    ...
    if (!articles || articles.length === 0) {
      articles = [
        { id: 1, titleTa: 'தமிழகத்தில் நாளை முதல் கனமழை எச்சரிக்கை – வானிலை மையம் அறிவிப்பு', viewsCount: 24500 },
        { id: 2, titleTa: 'நெல் கொள்முதல் விலை உயர்வு – விவசாயிகள் மகிழ்ச்சி', viewsCount: 19800 },
        { id: 3, titleTa: 'IPL 2026: சென்னை அணியின் புதிய கேப்டன் அறிவிப்பு', viewsCount: 16200 },
        { id: 4, titleTa: 'சென்னையில் தங்கம் விலை சவரனுக்கு ₹400 குறைந்தது', viewsCount: 14100 },
        { id: 5, titleTa: 'புதிய வந்தே பாரத் ரயில் சேவை தொடக்கம் – கட்டணம் விபரம்', viewsCount: 11900 }
      ];
    }
  ...
  // ===== 8. AGRICULTURE NEWS + MARKET RATES =====
  async function loadAgriSection() {
    try {
      const catId = await getCategoryIdBySlug('agriculture');
      let url = `${API_BASE}/api/v1/articles/getAllWeb?size=3`;
      ...
      if (res.ok) {
        ...
        const items = data.content || [];
        if (items.length > 0) {
          renderNewsCards(items, 'agriNewsGrid', '#16A34A', '\u0bb5\u0bbf\u0bb5\u0b9a\u0bbe\u0baf\u0bae\u0bcd');
        } else {
          renderFallbackAgriNews();
        }
      } else {
        renderFallbackAgriNews();
      }
    ...
    // Load market rates from API and update header widget
    try {
      const res = await fetch(`${API_BASE}/api/v1/market/live-rates`);
      if (res.ok) {
        const data = await res.json();
        if (data.gold) {
          const g22 = document.getElementById('gold22kVal');
          const g24 = document.getElementById('gold24kVal');
          const sil = document.getElementById('silverVal');
          const plat = document.getElementById('platVal');
          if (g22 && data.gold['22K']) g22.textContent = `₹${data.gold['22K']}/g`;
          if (g24 && data.gold['24K']) g24.textContent = `₹${data.gold['24K']}/g`;
          if (sil && data.silver) sil.textContent = `₹${data.silver.price || data.silver}/g`;
          if (plat && data.platinum) plat.textContent = `₹${data.platinum}/g`;
        }
      }
    } catch (err) { }

    // Bind location tag selector
    const distSel = document.getElementById('districtSelector');
    const distTag = document.getElementById('districtNameTag');
    const goldLocTag = document.getElementById('goldLocationLabel');
    if (distSel) {
      distSel.addEventListener('change', function() {
        const selected = this.value;
        if (distTag) distTag.textContent = `${selected} 24×7`;
        if (goldLocTag) goldLocTag.textContent = `${selected} Gold Rate`;
      });
    }
    ...
    try {
      ...
      const list = document.getElementById('marketRatesList');
      const lastUpdated = document.getElementById('marketLastUpdated');
      if (!list) return;

      const commodities = [];
      if (data.gold) {
        if (data.gold['22K']) commodities.push({ icon: '🪙', name: 'தங்கம் (22K)', price: data.gold['22K'], change: data.gold.change || '' });
        if (data.gold['24K']) commodities.push({ icon: '💰', name: 'தங்கம் (24K)', price: data.gold['24K'], change: '' });
      }
      if (data.silver) commodities.push({ icon: '⚪', name: 'வெள்ளி', price: data.silver.price || data.silver, change: '' });

      if (commodities.length > 0) {
        list.innerHTML = commodities.map(c => {
          const isUp = c.change && !c.change.toString().includes('-');
          const changeHtml = c.change ? `<span style="font-size:11px;color:${isUp ? '#16A34A' : '#EF4444'};font-weight:600;">${isUp ? '▲' : '▼'} ${c.change}</span>` : '';
          return `<div style="padding:10px;background:var(--bg-light);border-radius:8px;display:flex;flex-direction:column;gap:4px;">
            <span style="font-size:12px;color:var(--text-muted);">${c.icon} ${c.name}</span>
            <span style="font-size:16px;font-weight:800;color:var(--primary);">₹${c.price}</span>
            ${changeHtml}
          </div>`;
        }).join('');
        if (lastUpdated) lastUpdated.textContent = 'இன்று புதுப்பிக்கப்பட்டது';
      } else {
        loadDefaultMarketRates(list, lastUpdated);
      }
    } catch (err) {
      const list = document.getElementById('marketRatesList');
      const lastUpdated = document.getElementById('marketLastUpdated');
      if (list) loadDefaultMarketRates(list, lastUpdated);
    }
  }

  function renderFallbackAgriNews() {
    const dummyAgri = [
      { id: 101, titleTa: '🌾 நெல் கொள்முதல் விலை குவிண்டாலுக்கு ₹100 உயர்வு – அரசு அரசாணை வெளியீடு', shortDescTa: 'விவசாயிகளின் வாழ்வாதாரத்தை உயர்த்தும் வகையில் நெல் கொள்முதல் விலையை அதிகரித்து தமிழக அரசு ஆணை பிறப்பித்துள்ளது.', categoryNameTa: 'விவசாயம்', publishedAt: new Date().toISOString(), viewsCount: 1420 },
      { id: 102, titleTa: '💧 சொட்டு நீர் பாசனத்திற்கு 100% மானியம் – விவசாயிகள் உடனடியாக விண்ணப்பிக்க அழைப்பு', shortDescTa: 'வேளாண்மைத் துறை சார்பில் சொட்டு நீர் பாசனக் கருவிகள் அமைக்க சிறு விவசாயிகளுக்கு முழு மானியம் வழங்கப்படுகிறது.', categoryNameTa: 'விவசாயம்', publishedAt: new Date().toISOString(), viewsCount: 980 },
      { id: 103, titleTa: '🌱 இயற்கை விவசாயத்திற்கு மாறும் தமிழ்நாடு – புதிய கூட்டுறவு சந்தை தொடக்கம்', shortDescTa: 'கரிம விளைபொருட்களை நேரடியாக நுகர்வோருக்கு விற்க மாவட்டம்தோறும் இயற்கை விவசாய கூட்டுறவு அங்காடிகள் திறப்பு.', categoryNameTa: 'விவசாயம்', publishedAt: new Date().toISOString(), viewsCount: 1150 }
    ];
    renderNewsCards(dummyAgri, 'agriNewsGrid', '#16A34A', 'விவசாயம்');
  }

  function loadDefaultMarketRates(list, lastUpdated) {
    const rates = [
      { icon: '🌾', name: 'நெல் (பொது)', price: '₹2,280/kg', change: '+₹40', up: true },
      { icon: '🌻', name: 'கடலை', price: '₹5,600/kg', change: '+₹120', up: true },
      { icon: '🟡', name: 'மஞ்சள்', price: '₹12,800/kg', change: '+₹350', up: true },
      { icon: '🧵', name: 'பருத்தி', price: '₹7,450/kg', change: '-₹180', up: false },
      { icon: '🫘', name: 'துவரை', price: '₹8,920/kg', change: '+₹65', up: true },
      { icon: '🥥', name: 'தேங்காய்', price: '₹48/ஒன்று', change: '+₹2', up: true },
    ];
    list.innerHTML = rates.map(r => `
      <div style="padding:10px;background:var(--bg-light);border-radius:8px;display:flex;flex-direction:column;gap:4px;">
        <span style="font-size:12px;color:var(--text-muted);">${r.icon} ${r.name}</span>
        <span style="font-size:16px;font-weight:800;color:var(--primary);">${r.price}</span>
        <span style="font-size:11px;color:${r.up ? '#16A34A' : '#EF4444'};font-weight:600;">${r.up ? '▲' : '▼'} ${r.change}</span>
      </div>`).join('');
    if (lastUpdated) lastUpdated.textContent = 'இன்றைய விலை';
  }

  // ===== 9. BUSINESS DASHBOARD =====
  async function loadBusinessSection() {
    try {
      const catId = await getCategoryIdBySlug('business');
      let url = `${API_BASE}/api/v1/articles/getAllWeb?size=3`;
      if (catId) url += `&categoryId=${catId}`;
      const res = await fetch(url);
      if (res.ok) {
        ...
        const items = data.content || [];
        if (items.length > 0) {
          renderNewsCards(items, 'businessNewsGrid', '#0057FF', '\u0bb5\u0ba3\u0bbf\u0b95\u0bae\u0bcd');
        } else {
          renderFallbackBusinessNews();
        }
      } else {
        renderFallbackBusinessNews();
      }
  ...
  function renderFallbackBusinessNews() {
    const dummyBusiness = [
      { id: 201, titleTa: '📈 பங்குச்சந்தை புதிய சாதனை உச்சம் – முதலீட்டாளர்களுக்கு 2 லட்சம் கோடி லாபம்', shortDescTa: 'சென்செக்ஸ் மற்றும் நிஃப்டி புள்ளிகள் முன்னெப்போதும் இல்லாத வகையில் உயர்ந்து புதிய வரலாற்று மைல்கல்லை எட்டியது.', categoryNameTa: 'வணிகம்', publishedAt: new Date().toISOString(), viewsCount: 3100 },
      { id: 202, titleTa: '🏬 சிறு தொழில் முனைவோருக்கு ₹10 லட்சம் கடன் திட்டம் – புதிய மானிய வழிகாட்டுதல்', shortDescTa: 'MSME துறையை ஊக்குவிக்க பிணையில்லா கடன் திட்டத்தின் கீழ் கடன் வரம்பை உயர்த்தி மத்திய அரசு அறிவிப்பு.', categoryNameTa: 'வணிகம்', publishedAt: new Date().toISOString(), viewsCount: 2450 },
      { id: 203, titleTa: '🏭 சென்னையில் புதிய மின்சார வாகன உதிரிபாக ஆலை – ₹1500 கோடி முதலீடு', shortDescTa: 'சர்வதேச உற்பத்தி நிறுவனம் தமிழகத்தில் தனது மிகப்பெரிய மின்சார வாகன உற்பத்தி மையத்தை அமைக்கிறது.', categoryNameTa: 'வணிகம்', publishedAt: new Date().toISOString(), viewsCount: 1890 }
    ];
    renderNewsCards(dummyBusiness, 'businessNewsGrid', '#0057FF', 'வணிகம்');
  }

  // ===== 10. DISTRICT NEWS =====
  let _districtList = [];
  async function loadDistrictSection() {
    try {
      const res = await fetch(`${API_BASE}/api/v1/districts`);
      if (res.ok) {
        _districtList = await res.json();
        if (_districtList.length > 0) buildDistrictButtons(_districtList);
        else renderDefaultDistrictButtons();
      } else {
        renderDefaultDistrictButtons();
      }
    } catch (err) {
      renderDefaultDistrictButtons();
    }
    await loadDistrictNews(null);
  }

  function renderDefaultDistrictButtons() {
    _districtList = [
      ...
      { id: '5', nameEn: 'Trichy', nameTa: 'திருச்சி' },
    ];
    buildDistrictButtons(_districtList);
  }

  function buildDistrictButtons(districts) {
    const container = document.getElementById('districtFilterBtns');
    if (!container) return;
    const lang = Storage.get('lang', 'ta');
    const allBtn = `<button class="district-btn active" data-district-id="" style="padding:6px 14px;border-radius:20px;font-size:13px;font-weight:600;border:1px solid var(--primary);background:var(--primary);color:white;cursor:pointer;">அனைத்தும்</button>`;
    const distBtns = districts.slice(0, 6).map(d => {
      const name = lang === 'ta' ? (d.nameTa || d.nameEn || d.name) : (d.nameEn || d.name || d.nameTa);
      return `<button class="district-btn" data-district-id="${d.id || ''}" style="padding:6px 14px;border-radius:20px;font-size:13px;font-weight:600;border:1px solid var(--border);background:var(--white);color:var(--text-dark);cursor:pointer;">${sanitize(name)}</button>`;
    }).join('');
    container.innerHTML = allBtn + distBtns;

    container.querySelectorAll('.district-btn').forEach(btn => {
      btn.addEventListener('click', async function() {
        container.querySelectorAll('.district-btn').forEach(b => {
          b.style.background = 'var(--white)'; b.style.color = 'var(--text-dark)'; b.style.borderColor = 'var(--border)';
        });
        this.style.background = 'var(--primary)'; this.style.color = 'white'; this.style.borderColor = 'var(--primary)';
        await loadDistrictNews(this.dataset.districtId || null);
      });
    });
  }

  async function loadDistrictNews(districtId) {
    const grid = document.getElementById('districtNewsGrid');
    if (grid) grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:20px;"><i class="fas fa-spinner fa-spin" style="color:var(--primary);font-size:24px;"></i></div>';
    try {
      let url = `${API_BASE}/api/v1/articles/getAllWeb?size=3`;
      ...
      if (res.ok) {
        ...
        const items = data.content || [];
        if (items.length > 0) {
          renderNewsCards(items, 'districtNewsGrid', '#7C3AED', '\u0bae\u0bbe\u0bb5\u0b9f\u0bcd\u0b9f\u0bae\u0bcd');
        } else {
          renderFallbackDistrictNews(districtId);
        }
      } else {
        renderFallbackDistrictNews(districtId);
      }
  ...
  function renderFallbackDistrictNews(districtId) {
    const dummyDistrict = [
      { id: 301, titleTa: '🏙️ சென்னை மெட்ரோ ரயில் 2-ஆம் கட்டப் பணிகள் வேகம் – புதிய தடம் திறப்பு', shortDescTa: 'சென்னையில் போக்குவரத்து நெரிசலைக் குறைக்க மெட்ரோ ரயில் விரிவாக்கப் பணிகள் தீவிரமாக நடைபெற்று வருகின்றன.', categoryNameTa: 'மாவட்ட செய்திகள்', publishedAt: new Date().toISOString(), viewsCount: 4200 },
      { id: 302, titleTa: '🌆 கோவையில் புதிய தகவல் தொழில்நுட்ப பூங்கா – 20,000 பேருக்கு வேலைவாய்ப்பு', shortDescTa: 'கொடிசியாவில் அமையவுள்ள எல்காட் தொழில் நுட்ப வளாகத்திற்கு முதலமைச்சர் நேரில் அடிக்கல் நாட்டினார்.', categoryNameTa: 'மாவட்ட செய்திகள்', publishedAt: new Date().toISOString(), viewsCount: 2800 },
      { id: 303, titleTa: '🏛️ மதுரையில் சித்திரை திருவிழா சிறப்பு ஏற்பாடுகள் – காவல்துறை பாதுகாப்பு தீவிரம்', shortDescTa: 'உலகப் புகழ்பெற்ற மதுரை சித்திரை திருவிழாவை முன்னிட்டு பக்தர்கள் வசதிக்காக சிறப்பு பேருந்துகள் இயக்கம்.', categoryNameTa: 'மாவட்ட செய்திகள்', publishedAt: new Date().toISOString(), viewsCount: 3600 }
    ];
    renderNewsCards(dummyDistrict, 'districtNewsGrid', '#7C3AED', 'மாவட்டம்');
  }

  // ===== 11. ELECTION CENTER 2026 =====
  async function loadElectionSection() {
    try {
      const res = await fetch(`${API_BASE}/api/v1/election/results`);
      if (res.ok) {
        const results = await res.json();
        const items = Array.isArray(results) ? results : (results.content || []);
        renderElectionScoreboard(items);
      } else {
        renderElectionScoreboard([]);
      }
    } catch (err) {
      renderElectionScoreboard([]);
    }
    ...
    try {
      const catId = await getCategoryIdBySlug('election');
      let url = `${API_BASE}/api/v1/articles/getAllWeb?size=3`;
      if (catId) url += `&categoryId=${catId}`;
      const res = await fetch(url);
      if (res.ok) {
        ...
        const items = data.content || [];
        if (items.length > 0) {
          renderNewsCards(items, 'electionNewsGrid', '#EF4444', '\u0ba4\u0bc7\u0bb0\u0bcd\u0ba4\u0bb2\u0bcd');
        } else {
          renderFallbackElectionNews();
        }
      } else {
        renderFallbackElectionNews();
      }
  ...
  function renderFallbackElectionNews() {
    const dummyElection = [
      { id: 401, titleTa: '🗳️ சட்டமன்ற தேர்தல் 2026 – அனைத்து கட்சிகளும் தொகுதி பங்கீட்டு பேச்சுவார்த்தை தீவிரம்', shortDescTa: 'தமிழக சட்டமன்ற தேர்தலை முன்னிட்டு பிரதான கூட்டணிகள் தங்களின் தொகுதி பங்கீட்டு குழுக்களை அமைத்துள்ளன.', categoryNameTa: 'தேர்தல் மையம் 2026', publishedAt: new Date().toISOString(), viewsCount: 5200 },
      { id: 402, titleTa: '📋 வாக்காளர் பட்டியல் திருத்தம் – பெயர் சேர்க்கைக்கான சிறப்பு முகாம் அறிவிப்பு', shortDescTa: '18 வயது பூர்த்தியடைந்த புதிய வாக்காளர்கள் இணையவழியில் விண்ணப்பிக்க தேர்தல் ஆணையம் அறிவுறுத்தல்.', categoryNameTa: 'தேர்தல் மையம் 2026', publishedAt: new Date().toISOString(), viewsCount: 3400 },
      { id: 403, titleTa: '🎤 தேர்தல் பிரசாரம் சூடுபிடிப்பு – தலைவர்கள் தமிழகம் முழுவதும் தீவிர பயணம்', shortDescTa: 'மக்கள் சந்திப்பு பயணங்கள் மூலம் தங்கள் கூட்டணி சாதனைகளை விளக்கி தேர்தல் வாக்குறுதிகளை வெளியிடுகின்றனர்.', categoryNameTa: 'தேர்தல் மையம் 2026', publishedAt: new Date().toISOString(), viewsCount: 4100 }
    ];
    renderNewsCards(dummyElection, 'electionNewsGrid', '#EF4444', 'தேர்தல்');
  }

  function renderElectionScoreboard(results) {
    const container = document.getElementById('electionPartyList');
    if (!container) return;

    // If no results from API, show default placeholder parties
    const parties = results.length > 0 ? results : [
      { partyNameTa: '\u0ba4\u0bbf\u0bae\u0bc1\u0b95', partyColor: '#EF4444', seatsWon: 133, totalSeats: 234, voteSharePercent: 38.2 },
      { partyNameTa: '\u0b85\u0ba4\u0bbf\u0bae\u0bc1\u0b95', partyColor: '#1E3A8A', seatsWon: 65, totalSeats: 234, voteSharePercent: 25.6 },
      { partyNameTa: '\u0bae\u0b95\u0bcd\u0b95\u0bb3\u0bcd \u0ba8\u0bb2\u0ba9\u0bcd', partyColor: '#7C3AED', seatsWon: 18, totalSeats: 234, voteSharePercent: 8.4 },
    ];

    const totalSeats = parties[0] ? (parties[0].totalSeats || 234) : 234;
    container.innerHTML = parties.map(p => {
      const pct = p.voteSharePercent || Math.round((p.seatsWon / totalSeats) * 100);
      const name = p.partyNameTa || p.partyName || p.name || 'Party';
      const color = p.partyColor || '#0057FF';
      return `
        <div style="display:flex;align-items:center;gap:12px;">
          <div style="min-width:80px;font-size:13px;font-weight:700;color:${color};">${sanitize(name)}</div>
          <div style="flex:1;height:28px;background:var(--bg-light);border-radius:6px;overflow:hidden;position:relative;">
            <div style="height:100%;width:${pct}%;background:${color};border-radius:6px;transition:width 1s ease;display:flex;align-items:center;padding-left:8px;">
              <span style="font-size:11px;font-weight:700;color:white;white-space:nowrap;">${p.seatsWon || 0} \u0b87\u0b9f\u0bae\u0bcd (${pct}%)</span>
            </div>
          </div>
        </div>`;
    }).join('');
  ...
  async function loadActivePoll() {
    ...
    if (!container) return;
    try {
      const res = await fetch(`${API_BASE}/api/v1/polls/getAllWeb?size=1`);
      if (!res.ok) throw new Error('Poll API error');
      const data = await res.json();
      const polls = data.content || (Array.isArray(data) ? data : []);
      if (polls.length === 0) {
        container.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-muted);">\u0b8f\u0ba4\u0bc1\u0bae\u0bcd \u0b95\u0bb0\u0bc1\u0ba4\u0bcd\u0ba4\u0bc1\u0b95\u0bcd \u0b95\u0ba3\u0bbf\u0baa\u0bcd\u0baa\u0bc1 \u0b87\u0bb2\u0bcd\u0bb2\u0bc8</div>';
        return;
      }
      const poll = polls[0];
      _activePollId = poll.id;
      renderPoll(poll);
    } catch (err) {
      // Fallback static poll
      renderStaticPoll();
      console.warn('Poll API error:', err);
    }
  }
  ...
  function renderPoll(poll) {
    ...
    if (!container) return;
    const lang = Storage.get('lang', 'ta');
    const question = lang === 'ta' ? (poll.questionTa || poll.question) : (poll.question || poll.questionTa);
    
    const opts = [];
    if (poll.option1Ta || poll.option1) opts.push({ num: 1, text: lang === 'ta' ? (poll.option1Ta || poll.option1) : (poll.option1 || poll.option1Ta), votes: poll.option1Votes || 0 });
    if (poll.option2Ta || poll.option2) opts.push({ num: 2, text: lang === 'ta' ? (poll.option2Ta || poll.option2) : (poll.option2 || poll.option2Ta), votes: poll.option2Votes || 0 });
    if (poll.option3Ta || poll.option3) opts.push({ num: 3, text: lang === 'ta' ? (poll.option3Ta || poll.option3) : (poll.option3 || poll.option3Ta), votes: poll.option3Votes || 0 });
    
    const totalVotes = opts.reduce((s, o) => s + o.votes, 0);
    const hasVoted = localStorage.getItem(`poll_voted_${poll.id}`) === '1';

    container.innerHTML = `
      <h4 style="font-size:16px;font-weight:700;margin-bottom:16px;line-height:1.5;">${sanitize(question)}</h4>
      <form id="activePollForm">
        ${opts.map(o => {
          const pct = totalVotes > 0 ? Math.round((o.votes / totalVotes) * 100) : 0;
          return `<div style="margin-bottom:12px;">
            <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:10px;border-radius:8px;border:1px solid var(--border);transition:var(--transition);" onmouseover="this.style.background='var(--bg-light)'" onmouseout="this.style.background='transparent'">
              <input type="radio" name="pollOpt" value="${o.num}" ${hasVoted ? 'disabled' : ''} style="accent-color:var(--primary);">
              <span style="flex:1;font-size:14px;font-weight:500;">${sanitize(o.text)}</span>
              ${hasVoted ? `<div style="min-width:80px;"><div style="height:6px;background:var(--bg-light);border-radius:3px;overflow:hidden;"><div style="height:100%;width:${pct}%;background:var(--primary);border-radius:3px;"></div></div><span style="font-size:11px;color:var(--text-muted);">${pct}%</span></div>` : ''}
            </label>
          </div>`;
        }).join('')}
        ${!hasVoted ? `<button type="submit" style="padding:10px 24px;background:var(--primary);color:white;border:none;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;width:100%;margin-top:8px;">\u0bb5\u0bbe\u0b95\u0bcd\u0b95\u0bb3\u0bbf\u0b95\u0bcd</button>` : `<p style="text-align:center;font-size:13px;color:var(--text-muted);margin-top:8px;">\u0b89\u0b99\u0bcd\u0b95\u0bb3\u0bcd \u0bb5\u0bbe\u0b95\u0bcd\u0b95\u0bc1 \u0baa\u0ba4\u0bbf\u0bb5\u0bc1 \u0b9a\u0bc6\u0baf\u0bcd\u0baf\u0baa\u0bcd\u0baa\u0b9f\u0bcd\u0b9f\u0ba4\u0bc1. \u0ba8\u0ba9\u0bcd\u0bb1\u0bbf!</p>`}
      </form>
      <p style="font-size:11px;color:var(--text-muted);margin-top:12px;text-align:right;">\u0bae\u0bca\u0ba4\u0bcd\u0ba4\u0bae\u0bcd: ${totalVotes} \u0bb5\u0bbe\u0b95\u0bcd\u0b95\u0bc1\u0b95\u0bb3\u0bcd</p>`;

    const form = document.getElementById('activePollForm');
    if (form && !hasVoted) {
      form.addEventListener('submit', async function(e) {
        e.preventDefault();
        const selected = this.querySelector('input[name="pollOpt"]:checked');
        if (!selected) { alert('\u0ba4\u0baf\u0bb5\u0bc1\u0b9a\u0bc6\u0baf\u0bcd\u0ba4\u0bc1 \u0b92\u0bb0\u0bc1 \u0bb5\u0bbf\u0bb0\u0bc1\u0baa\u0bcd\u0baa\u0ba4\u0bcd\u0ba4\u0bc8\u0ba4\u0bcd \u0ba4\u0bc7\u0bb0\u0bcd\u0ba8\u0bcd\u0ba4\u0bc6\u0b9f\u0bc1\u0b95\u0bcd\u0b95\u0bb5\u0bc1\u0bae\u0bcd'); return; }
        try {
          const optNum = parseInt(selected.value);
          await fetch(`${API_BASE}/api/v1/polls/${_activePollId}/vote?optionNum=${optNum}`, { method: 'POST' });
          localStorage.setItem(`poll_voted_${_activePollId}`, '1');
          // Re-fetch poll to show updated results
          const upd = await fetch(`${API_BASE}/api/v1/polls/getAllWeb?size=1`);
          if (upd.ok) { const d = await upd.json(); const ps = d.content || []; if (ps.length > 0) renderPoll(ps[0]); }
        } catch (err) {
          alert('\u0bb5\u0bbe\u0b95\u0bcd\u0b95\u0bb3\u0bbf\u0b95\u0bcd\u0b95\u0bc1\u0bae\u0bcd\u0baa\u0bcb\u0ba4\u0bc1 \u0baa\u0bbf\u0bb4\u0bc8 \u0b8f\u0bb1\u0bcd\u0baa\u0b9f\u0bcd\u0b9f\u0ba4\u0bc1. \u0bae\u0bc0\u0ba3\u0bcd\u0b9f\u0bc1\u0bae\u0bcd \u0bae\u0bc1\u0baf\u0bb1\u0bcd\u0b9a\u0bbf\u0b95\u0bcd\u0b95\u0bb5\u0bc1\u0bae\u0bcd.');
        }
      });
    }
  }

  function renderStaticPoll() {
    const container = document.getElementById('pollContainer');
    if (!container) return;
    container.innerHTML = `
      <h4 style="font-size:16px;font-weight:700;margin-bottom:16px;">\u0ba4\u0bae\u0bbf\u0bb4\u0b95\u0ba4\u0bcd\u0ba4\u0bbf\u0bb2\u0bcd \u0b85\u0b9f\u0bc1\u0ba4\u0bcd\u0ba4 \u0bae\u0bc1\u0ba4\u0bb2\u0bae\u0bc8\u0b9a\u0bcd\u0b9a\u0bb0\u0bcd \u0baf\u0bbe\u0bb0\u0bcd?</h4>
      <div style="padding:20px;text-align:center;color:var(--text-muted);">\u0b95\u0bb0\u0bc1\u0ba4\u0bcd\u0ba4\u0bc1\u0b95\u0bcd \u0b95\u0ba3\u0bbf\u0baa\u0bcd\u0baa\u0bc1 \u0bb5\u0bbf\u0bb0\u0bc8\u0bb5\u0bbf\u0bb2\u0bcd \u0bb5\u0bb0\u0bc1\u0bae\u0bcd.</div>`;
  }
  ...
  function setupNewsletterForm() {
    ...
    form.addEventListener('submit', async function(e) {
      ...
      const submitBtn = document.getElementById('newsletterSubmitBtn') || this.querySelector('button[type="submit"]');
      const msgEl = document.getElementById('newsletterMsg');
      const email = emailInput ? emailInput.value.trim() : '';
      if (!email) return;

      if (submitBtn) { submitBtn.disabled = true; submitBtn.textContent = '\u0baa\u0ba4\u0bbf\u0bb5\u0bc1 \u0b9a\u0bc6\u0baf\u0bcd\u0b95\u0bbf\u0bb1\u0ba4\u0bc1...'; }
      try {
        ...
        if (res.ok || res.status === 201) {
          if (msgEl) { msgEl.textContent = '\u0bb5\u0bc6\u0bb1\u0bcd\u0bb1\u0bbf\u0b95\u0bb0\u0bae\u0bbe\u0b95 \u0baa\u0ba4\u0bbf\u0bb5\u0bc1 \u0b9a\u0bc6\u0baf\u0bcd\u0baf\u0baa\u0bcd\u0baa\u0b9f\u0bcd\u0b9f\u0ba4\u0bc1! \u0ba8\u0ba9\u0bcd\u0bb1\u0bbf.'; msgEl.style.display = 'block'; }
          if (emailInput) emailInput.value = '';
        } else {
          if (msgEl) { msgEl.textContent = '\u0baa\u0ba4\u0bbf\u0bb5\u0bc1 \u0b9a\u0bc6\u0baf\u0bcd\u0baf \u0bae\u0bc1\u0b9f\u0bbf\u0baf\u0bb5\u0bbf\u0bb2\u0bcd\u0bb2\u0bc8. \u0bae\u0bc0\u0ba3\u0bcd\u0b9f\u0bc1\u0bae\u0bcd \u0bae\u0bc1\u0baf\u0bb1\u0bcd\u0b9a\u0bbf\u0b95\u0bcd\u0b95\u0bb5\u0bc1\u0bae\u0bcd.'; msgEl.style.display = 'block'; }
        }
      } catch (err) {
        if (msgEl) { msgEl.textContent = '\u0b87\u0ba3\u0bc8\u0baa\u0bcd\u0baa\u0bc1 \u0baa\u0bbf\u0bb4\u0bc8. \u0bae\u0bc0\u0ba3\u0bcd\u0b9f\u0bc1\u0bae\u0bcd \u0bae\u0bc1\u0baf\u0bb1\u0bcd\u0b9a\u0bbf\u0b95\u0bcd\u0b95\u0bb5\u0bc1\u0bae\u0bcd.'; msgEl.style.display = 'block'; }
      } finally {
        if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = '\u0baa\u0ba4\u0bbf\u0bb5\u0bc1 \u0b9a\u0bc6\u0baf\u0bcd\u0b95'; }
      }
    });
  }
  ...
  // Phase 1: Load dynamic data (quick wins)
  loadBreakingNews();
  loadHeroSection();
  loadLatestNews();
  loadVideoNews();
  ...
  // Phase 2: Load category-filtered sections
  loadAgriSection();
  loadBusinessSection();
  loadDistrictSection();
  loadElectionSection();
